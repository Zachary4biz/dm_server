...
class YOLO(object):
    _defaults = {
        ...
        "model_path": os.path.dirname(__file__)+"/../CVServer/apps/obj_detection"+'/model/yolo.h5',
        "anchors_path": os.path.dirname(__file__)+"/../CVServer/apps/obj_detection"+'/model/original_anchors.txt',
        "classes_path": os.path.dirname(__file__)+"/../CVServer/apps/obj_detection"+'/model/original_classes.txt',
        "score": 0.3,
        "iou": 0.45,
...
def detect_video(yolo, video_path, output_path=""):
    import cv2
    # 改成0摄像头 路径是视频
    vid = cv2.VideoCapture(0)  # video_path
    if not vid.isOpened():
...
def detect_img_from_url(yolo,default_url=None):
    if default_url is not None:
        image = img_from_url_cv2(default_url)  # 402,600,3
        image = cv2.cvtColor(cv2.resize(image, (416, 416)), cv2.COLOR_BGR2RGB)
        image = Image.fromarray(image)
        with yolo.graph.as_default():
            r_image, class_found = yolo.detect_image(image)
        class_found = json.dumps(dict([(k, len(list(g))) for k, g in itertools.groupby(sorted(class_found))]))
        r_image.show()
        print(">>>>>> found classes: {}".format(class_found))
